...
let slotAutoRefreshInterval = null;

function startSlotAutoRefresh() {
    if (slotAutoRefreshInterval) return; // already running
    slotAutoRefreshInterval = setInterval(() => {
        const profile = (() => {
            const sel = document.getElementById("web-farm-profile-select");
            return sel ? sel.value || "profile_1" : "profile_1";
        })();
        fetch(getApiUrl(`/api/web_farming_status?profile=${profile}`))
        .then(res => res.json())
        .then(data => { renderWebFarmSlots(data); })
        .catch(() => {});
    }, 5000);
}

function stopSlotAutoRefresh() {
    if (slotAutoRefreshInterval) {
        clearInterval(slotAutoRefreshInterval);
        slotAutoRefreshInterval = null;
    }
}
        ...
        if (select) {
            ...
            if (data.profiles && data.profiles.length > 0) {
                data.profiles.forEach(p => {
                    ...
                    let label = p === 'personal' ? 'เบราว์เซอร์ปกติประจำวัน (Personal)' : `Profile: ${p}`;
...
function renderWebFarmSlots(data) {
    ...
    if (profiles.length === 0) {
        gridEl.innerHTML = `
            <div style="grid-column: 1/-1; text-align: center; padding: 40px 24px; color: var(--text-dim); border: 1px dashed rgba(255,255,255,0.12); border-radius: 12px; background: rgba(255,255,255,0.02);">
                <div style="font-size: 32px; margin-bottom: 12px;">🗂️</div>
                <div style="font-family: 'IBM Plex Mono', monospace; font-size: 13px; color: var(--neon-cyan); margin-bottom: 8px; font-weight: bold;">ไม่มีโปรไฟล์ในระบบ</div>
                <div style="font-size: 11px; margin-bottom: 20px; line-height: 1.6;">ลบโปรไฟล์ทั้งหมดออกแล้ว<br>กรอกชื่อโปรไฟล์ด้านบนแล้วกด <strong style="color:var(--neon-yellow)">สร้างโปรไฟล์</strong> เพื่อเริ่มใหม่</div>
            </div>
        `;
        if (summaryEl) summaryEl.innerText = 'ไม่มีโปรไฟล์ — กรุณาสร้างใหม่';
        return;
    }
        ...
        card.innerHTML = `
            <div class="slot-header">
                ...
                <div style="display: flex; align-items: center; gap: 6px;">
                    ...
                    <button onclick="deleteWebFarmProfile('${p}')" title="ลบโปรไฟล์นี้" style="background: rgba(255,60,60,0.12); border: 1px solid rgba(255,60,60,0.35); padding: 3px 8px; color: #ff5555; cursor: pointer; border-radius: 4px; font-size: 10px; font-family: 'IBM Plex Mono', monospace; font-weight: bold; letter-spacing: 0.5px; transition: all 0.18s;" onmouseover="this.style.background='rgba(255,60,60,0.28)'; this.style.color='#ff2222';" onmouseout="this.style.background='rgba(255,60,60,0.12)'; this.style.color='#ff5555';">
                        ✕ ลบ
                    </button>
                </div>
            </div>
...
function openUploaderLoginChrome() {
