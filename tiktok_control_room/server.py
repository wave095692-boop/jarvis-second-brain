...
class SecondBrainHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/api/status':
            ...
            # No auto-seeding — let profiles be managed manually by user
        ...
        if path == '/api/notes':
            ...
            try:
                ...
                if action == 'regenerate_pdf':
                    ...
                    # Use 'open -a' on macOS for more reliable launch, with args passed via --args
                    ...
                    try:
                        ...
                        log_message = f"Opened isolated Chrome browser for manual login (profile: {profile}). Please log in directly in the opened window. Once logged in, you can close it and click 'START' in the dashboard."
                        import time
                        time.sleep(1.5)
